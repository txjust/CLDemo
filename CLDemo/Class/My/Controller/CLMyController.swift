...
class CLMyController: CLBaseViewController {
    lazy var titleView: CLTitleControllerView = {
        let view: CLTitleControllerView = CLTitleControllerView(frame: CGRect(x: 0, y: cl_safeAreaInsets().top + cl_statusBarHeight(), width: self.view.bounds.width, height: self.view.bounds.height - cl_safeAreaInsets().top - cl_statusBarHeight() - cl_safeAreaInsets().bottom - (self.tabBarController?.tabBar.bounds.height ?? 0.0)), dataSource: self)
        return view
    }()
